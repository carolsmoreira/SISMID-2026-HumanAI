# Activity 07: XGBoost quantile forecast.
#
...
#
# All outputs are written to Activity 07-specific folders; no baseline,
# Activity 05, Activity 06, or Activity 08 artifact is touched.
#
# See rules.md and AGENTS.md.

# ---- Required packages -------------------------------------------------------
pkgs <- c("readr", "dplyr", "tidyr", "lubridate", "ggplot2", "MMWRweek", "xgboost", "scoringutils")
for (p in pkgs) {
...
score_quantile_forecasts <- function(fc, truth_df, model_name) {
  ...
  wide <- fc %>%
    inner_join(truth_df %>% select(week, location, observed = value),
               by = c("target_end_date" = "week", "location")) %>%
  ...
  required_q_names <- paste0("q", quantile_ladder)
  if (!all(required_q_names %in% names(wide))) {
    stop("Missing required quantile columns for ", model_name)
  }

  q_matrix <- as.matrix(wide[, required_q_names])
  wis_values <- scoringutils::wis(wide$observed, q_matrix, quantile_ladder, count_median_twice = TRUE)
  ae_values <- scoringutils::ae_median_quantile(wide$observed, q_matrix, quantile_ladder)
  coverage_values <- scoringutils::interval_coverage(wide$observed, q_matrix, quantile_ladder, interval_range = 95)
  ...
  scores <- wide %>%
    transmute(
      ...
      observed,
      median = q0.5,
      absolute_error = ae_values,
      squared_error = (median - observed)^2,
      wis = wis_values,
      lower_95 = q0.025,
      upper_95 = q0.975,
      coverage_95 = coverage_values,
...
if (file.exists(activity5_forecast_csv)) {
  activity5_fc <- read_csv(activity5_forecast_csv, show_col_types = FALSE)
  comparison_tables <- c(comparison_tables, list(score_quantile_forecasts(activity5_fc, df, "Activity 5 ensemble")$summary))
}
...
comparison_plot <- ggplot(comparison_plot_data, aes(x = horizon_label, y = value, fill = model)) +
  ...
  labs(
    ...
    y = NULL,
    fill = "Model",
    title = "Activity 07 XGBoost Forecast Comparison",
...
width_plot <- xgb_scored$scores %>%
  ...
  labs(
    ...
    title = "XGBoost Interval Widths by Horizon",
...
feature_data <- df %>%
  ...
  select(week, all_of(FEATURE_NAMES)) %>%
  pivot_longer(-week, names_to = "feature", values_to = "value") %>%
  filter(!is.na(value))

feature_plot <- ggplot(feature_data, aes(x = week, y = as.numeric(scale(value)), color = feature)) +
  geom_line(linewidth = .65) +
  facet_wrap(~feature, scales = "free_y", ncol = 2) +
  labs(
    x = "Week",
    y = "Standardized feature value",
    title = "XGBoost Feature Overview",
    subtitle = "Admissions lags and target-week seasonal harmonics used by the direct horizon models."
  ) +
  theme_minimal() +
